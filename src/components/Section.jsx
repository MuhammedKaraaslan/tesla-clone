...
function Section({ title, description, backgroundImg, leftButtonText, rightButtonText }) {
    return (
        <Wrap backgroundImage={backgroundImg}>
            {/* React Reveal is an animation library for React */}
            <Fade bottom>
                <ItemText>
                    <h1>{title}</h1>
                    <p>{description}</p>
            ...
            <Buttons>
                <Fade bottom>
                    <ButtonGroup>
                        ...
                        {/* Right button will only exists if there is a right button text*/}
...
const DownArrow = styled.img`
    height: 40px;
    overflow-x: hidden;   
    animation: animateDown infinite 1.5s;  //Created in index.css
`;
